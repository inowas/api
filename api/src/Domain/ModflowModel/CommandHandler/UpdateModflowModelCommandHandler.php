...
class UpdateModflowModelCommandHandler
{
    ...
    public function __invoke(UpdateModflowModelCommand $command)
    {
        ...
        $event = ModflowModelHasBeenUpdated::fromParams($userId, $modelId, $name, $description, $isPublic, $discretization);
        $modflowModelAggregate->apply($event);
        $this->aggregateRepository->storeEvent($event);
        $this->modflowModelProjector->apply($event);

        # Send command to update a toolInstance
        $command = UpdateToolInstanceCommand::fromParams($modelId, $name, $description, $isPublic, []);
